...
3. [Undervolting](#Подбор-оптимальных-значений-смещения-напряжений-на-вашем-процессоре-undervolting)
4. [О пост-кодах](#О-пост-кодах)
5. [Если вы наблюдаете частоту системной шины меньшую, чем 99,75МГц](#Если-вы-наблюдаете-частоту-системной-шины-меньшую-чем-9975МГц)
6. [Настройка управления оборотами вентиляторов для Huananzhi X99-8M/8MD3/F8/T8/TF](#Настройка-управления-оборотами-вентиляторов-для-Huananzhi-X99-8M8MD3F8T8TF)
...
5. В S3TurboTool нажимаем AMIBCP5
6. В появившейся утилите AMIBCP открываем биос
7. Раскрываем список и идём по пути "Common RefCode Configuration > IntelRCSetup > Advanced Power Management Configuration > CPU C State Control"
...
10. В S3TurboTool нажимаем "Собрать драйвер"
11. Настраиваем необходимые смещения напряжения (ниже есть методика нахождения примерных значений). Также выбираем, нужен ли дополнительный сигнал при включении и выводе системы из сна. Нажимаем "Собрать драйвер".
12. В S3TurboTool нажимаем UEFITool
13. В появившейся утилите UEFITool открываем биос
14. Раскрываем список и идём по пути "Intel image > BIOS region > 8C8CE578-...(самый нижний) >"
15. Примерно среди первых 20 значений находим 271DD6F2-... (модуль PchS3Peim)
16. Правый клик по нему, нажимаем "Replace as is...", выбираем собранный ранее драйвер (находится в папке S3TurboHack)

![](https://github.com/Koshak1013/HuananzhiX99_BIOS_mods/raw/master/screen01.png)

17. Выбираем "File > Save image file...", сохраняем
...
***Ответ:*** Можно снизить напряжение на процессоре не используя анлок, для этого не выполняем шаги 1-9 инструкции и на 11 шаге снимаем галку "Разблокировка турбо". Также есть опция "Баг SVID", это исказит отображение реального потребления процессора, в связи с чем у него пропадёт грань TDP. Рекомендуется только для тех, кто понимает зачем ему это нужно, т.к. максимально возможная нагрузка на компоненты значительно вырастет.

***Вопрос:*** Кошак, как тебе задонатить на видеокарту?

***Ответ:*** ^-^ Qiwi +790221333десять или 42766900десять267273. Большое спасибо!
