...
Далее переходим к тесту кэша (начать можно с -125mV). Тестировать кэш оптимально в LinX (проверено на v0.6.5), выставляем настройку 8192МБ памяти, нажимаем Start. При нестабильности возможен синий экран (обычно ошибка WHEA_UNCORRECTABLE_ERROR). Достаточно 5 минут теста.
...
4. Правый клик по "PE32 image section", нажимаем "Extract body...", сохраняем модуль с соответсвующим названием, чтобы не запутаться. Также делаем и со вторым модулем.
...
8. Теперь снова открываем UEFITool, делаем правый клик по "PE32 image section", нажимаем "Replace body...", и выбираем соответсвующий модуль. Также делаем и со вторым модулем.
9. Выбираем "File > Save image file...", сохраняем. Биос готов к прошивке. Прошить можно также соответсвующей кнопкой в S3TurboTool.
